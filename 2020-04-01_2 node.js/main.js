...
var path = require('path'); // 보안을 위해 사용... (../ 막아줌) --> 사용자들의 시스템 탐색을 막아줌(입력정보에 대한 보안) 즉, url에서 ../로 다른 파일로 접근 불가능 
// 객체 :  서로 연관된 데이터와 그 데이터를 처리하는 방법인 함수를 그룹핑해서 코드의 복잡성을 낮추는 수납상자
//refactoring --> 동작방식은 유지하면서 내부의 코드를 효율적으로 바꾸는 행위 
var template = require('./lib/template.js');
var sanitizeHtml = require('sanitize-html'); //--> 웹브라우저의 입력값에 태그가 있으면 태그를 지워버림
    ...
    if(pathname ==='/'){
    	if(queryData.id === undefined){ // undifned : 정의되지 않은 데이터 -> query String에 id 가 없다면 --> home
    		...
    		fs.readdir('./data', function(error, filelist){
            	...
            	var filterdPath = path.parse(queryData.id).base;//입력 보안
    			fs.readFile(`data/${filterdPath}`,'utf8',function(err,data){
    			var title = queryData.id;
    			var sanitizeHtmlTitle = sanitizeHtml(title); // title 값에 태그가 있을경우 태그를 지워버림
    			var sanitizeHtmldata = sanitizeHtml(data, {allowedTags:['h1']});// // data 값에 태그가 있을경우 태그를 지워버림
    			var list = template.list(filelist); // list를 함수로 처리한다.
    			var html = template.html(title, list, 
               			`<h2>${sanitizeHtmlTitle}</h2>${sanitizeHtmldata}`,	 // data 값
               			`<a href="/create">create</a> <a href="/update?id=${sanitizeHtmlTitle}">update</a> 
               			<form action="delete_process" method="post">
               				<input type="hidden" name="id" value="${sanitizeHtmlTitle}">
    	...
    	request.on('end', function(){
    		...
    		var id = post.id;
    		var filterdPath = path.parse(id).base;
    		fs.unlink(`data/${filterdPath}`, function(err){ // fs.unlink : (`삭제할 파일`, callback)
        		console.log(err);
        	})
